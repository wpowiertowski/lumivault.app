...
- [ ] **PipelinedImportCoordinator end-to-end** — primitives and phase-skipping wiring are now covered (AsyncPrimitivesTests, PipelinePhaseRoutingTests, PipelineItemTests, EnsureFileMirroredTests). What remains — sentinel task, per-stage cancellation, `copyError` vs `error` isolation, `defer`-finish on abnormal exit — needs protocol-based injection of the six services the coordinator constructs. Medium effort.
- [ ] **SyncCoordinator orchestration** — hydration and legacy catalog migration are now covered (HydrationTests, CatalogMigrationTests) via static, context-injected entry points. The rest (iCloud monitoring, settings debounce, `pushAfterLocalChange`) still orchestrates 3 services + UserDefaults + SwiftData and needs dependency injection. Medium effort.
- [ ] **ThumbnailService cache logic** — the on-disk contract is covered (ThumbnailCacheTests: Application Support root, sha-sharded layout, miss reads nil, removal). The NSCache layer and regeneration-from-a-mounted-volume still need real image rendering; CIContext renders all-white at small sizes in headless CI. Low value vs manual QA.
- [ ] **Hydration complexity guard (O(N) vs O(N²))** — blocked, not merely unwritten. A ratio assertion was implemented and removed after CI measured 4x the catalog costing 8.5x the time *with* the batch-load fix in place; SwiftData's per-insert cost grows with store size, so no threshold separates the fixed shape from the quadratic one. Counting `FetchDescriptor` executions would be the right guard, but `ModelContext` offers no seam to intercept.
