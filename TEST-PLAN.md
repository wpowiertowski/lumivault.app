...
| Medium Value | B2ServiceHelperTests | 3 | SHA-1 known test vectors (empty and non-empty), plus a three-row `@Test(arguments:)` table mapping HTTP status to outcome: success at 299 (the range's upper edge), 401 with a message body, and 500 with none (which proves the message stays nil rather than becoming an empty string). |
| High Value | PipelineOrchestrationTests | 13 | The real eight-stage import pipeline end to end via `importFiles`: persistence + album + catalog + bytes on the volume, PAR2 recovery volumes mirrored beside the index, ciphertext really written (not just the record flags), converted extension agreed on by record/catalog/disk, exact dedup, second-album membership, a failing volume copy reported without dropping the import, an unreadable source skipped without stopping siblings, cancellation both stopping short of the backlog and still persisting what was already archived, with no album-less records left behind (one test: the fixed-sleep variant was removed as a strictly weaker duplicate), counters reconciling with what was persisted, both halves of the album-deletion rule, and the Photos re-sync removal keeping an image that is still filed under another album (the fourth delete path, missed when membership became many-to-many). |
...
| Medium Value | PerceptualHashTests | 2 | Two functions, five cases. A four-row `@Test(arguments:)` table covers both extremes (0 and 64 bits), a known mid-range value and the invalid-length guard; the misaligned-slice regression that used to trap on SwiftData reads is a different shape and stays its own test. |
...
| Medium Value | EXIFDataFormattingTests | 2 | Two functions, four cases. A three-row `@Test(arguments:)` table covers sub-second, long exposure and zero (which used to trap on `Int(round(1.0/0))`); the unset-property case asserts on a value never assigned, so it has no row to sit in. |
